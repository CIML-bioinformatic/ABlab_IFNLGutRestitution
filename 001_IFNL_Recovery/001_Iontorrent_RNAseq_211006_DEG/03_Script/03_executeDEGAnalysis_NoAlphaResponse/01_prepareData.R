...
# Remove genes from the listed HALLMARKS
for( hallmark in HALLMARKS_TO_REMOVE){
  hallmark_gene_list = HALLMARKS_LIST_MOUSE[[ hallmark]]
  expression_data_chip1_df = expression_data_chip1_df[ !( row.names( expression_data_chip1_df) %in% hallmark_gene_list), ]
  expression_data_chip2_df = expression_data_chip2_df[ !( row.names( expression_data_chip2_df) %in% hallmark_gene_list), ]
}

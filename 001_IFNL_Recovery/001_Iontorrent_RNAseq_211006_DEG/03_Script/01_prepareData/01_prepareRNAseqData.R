...
cat("<BR><BR>Experiment meta-data loaded from", PATH_RNASEQ_METADATA)
...
# Define a string (ExperimentID) to summarize the experiment conditions+replicate
...
# Define a string (Condition) to summarize the experiment conditions
original_rnaseq_metadata$Condition = paste0( original_rnaseq_metadata$genotype, "_",
                                                original_rnaseq_metadata$treatment)

# Plot the datatable of meta-data
DT::datatable( original_rnaseq_metadata)

# Export the modified meta-data to file
export_rnaseq_metadata_file_path = file.path( PATH_ANALYSIS_OUTPUT, 
                                           paste0( 
                                             tools::file_path_sans_ext( basename( PATH_RNASEQ_METADATA)), 
                                             ".txt"))
cat("<BR>Export RNA-seq meta-data to file:", export_rnaseq_metadata_file_path)

write.table( original_rnaseq_metadata,
             file = export_rnaseq_metadata_file_path,
             col.names = TRUE,
             row.names = FALSE,
             sep="\t",
             quote=FALSE)
...
# Assign the "Gene" column as row names and remove the Gene column
row.names( original_rnaseq_chip1) = original_rnaseq_chip1$Gene
original_rnaseq_chip1$Gene = NULL

row.names( original_rnaseq_chip2) = original_rnaseq_chip2$Gene
original_rnaseq_chip2$Gene = NULL

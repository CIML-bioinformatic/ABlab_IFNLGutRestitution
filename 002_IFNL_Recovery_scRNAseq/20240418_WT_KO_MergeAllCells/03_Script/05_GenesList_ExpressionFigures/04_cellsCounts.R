...
for(currentListName in names(GENES_LIST))
{
  ...
  if(any(lowExpectedCountsGenes))
  {
    ...
    print( knitr::kable( addmargins(round(chisq[["expected"]],2)),
                         align   = "c",
                         caption = "Expected number of cells by population (Cluster) and condition (HTO) from X² (after filtering).\nTo compare with observed counts.")) #%>% kable_styling(bootstrap_options = c("condensed"), full_width = FALSE);
...
pathViolinOutput = file.path( PATH_ANALYSIS_OUTPUT,
                              "violinPlots")
dir.create(pathViolinOutput, recursive = TRUE, showWarnings = FALSE)
...
for(currentListName in names(GENES_LIST))
{
  ...
  filenamePNG = file.path( pathViolinOutput,
                           paste0( "cellsCounts_violin_byCluster_",  
  ...
  if(FLAG_secondRun) # Check if this is a second run for rendering only (see analysisParams)
  {
    ...
    cat( paste0( "\n![Violin - List:", currentListName, "](", filenamePNG,")\n" ))
  } else # If it's the first run, create the figure as external file
  {
    ...
    png( filenamePNG, 
         width = 700, 
         height = length(GENES_LIST[[currentListName]])*(nlevels(factor(Idents(sc10x))))*18)
...
for(currentListName in names(GENES_LIST))
{
  ...
  for(currentCluster in levels( Idents( sc10x)))
  {
    ...
    filenamePNG = file.path( pathViolinOutput,
                             paste0( "cellsCounts_violin_eachCluster_byHTO_",  
                                     path_sanitize(paste0(currentListName, "_", currentCluster), replacement = "_"), 
                                     ".png"))
    ...
    if(FLAG_secondRun) # Check if this is a second run for rendering only (see analysisParams)
    {
      # Don't mess with RMD layout using 'png' and 'dev.off'...
      # Integrate the figure that was generated during first run
      cat( paste0( "\n![Violin - List:", currentListName, " - Cluster:", currentCluster, "](", filenamePNG,")\n" ))
    } else # If it's the first run, create the figure as external file
    {
      # First run, generate figure as external file (messes up layout: figures in wring tabset)
      # Don't include them in report, will be done in secind run.
      png( filenamePNG, 
           width = 750, 
           height = length(GENES_LIST[[currentListName]])*(nlevels(factor(sc10x[["factorHTO", drop = TRUE]])))*30)
      print(countsBoxplots)
      dev.off()
      cat("\n\nFirst (computing) run. Run report generation a second time to get all tables/figures and proper layout...")
    }
...
# Create dir where png figures will be stored (and searched for in second run)
pathBubblesOutput = file.path( PATH_ANALYSIS_OUTPUT,
                              "bubblesPlots")
dir.create(pathBubblesOutput, recursive = TRUE, showWarnings = FALSE)
...
for(currentListName in names(GENES_LIST))
{
  ...
  for(currentCluster in levels( Idents( sc10x)))
  {
    ...
    # Plot
    figurePlot = ( ggplot( expressionSummaryDF,
                   aes( x = HTO, 
                        y = geneName)) +
             ...
             labs( x = "Cluster", 
                   y = "") +
             facet_wrap( vars(Cluster), ncol =1, strip.position = "right") + theme_minimal())
    
  
    filenamePNG = file.path( pathBubblesOutput,
                             paste0( "expression_bubbles_eachCluster_byHTO_",  
                                     path_sanitize(paste0(currentListName, "_", currentCluster), replacement = "_"), 
                                     ".png"))
    ...
    if(FLAG_secondRun) # Check if this is a second run for rendering only (see analysisParams)
    {
      # Don't mess with RMD layout using 'png' and 'dev.off'...
      # Integrate the figure that was generated during first run
      cat( paste0( "\n![Bubbles - List:", currentListName, " - Cluster:", currentCluster, "](", filenamePNG,")\n" ))
    } else # If it's the first run, create the figure as external file
    {
      # First run, generate figure as external file (messes up layout: figures in wring tabset)
      # Don't include them in report, will be done in secind run.
      png( filenamePNG, 
           width = ((nlevels(factor(sc10x[["factorHTO", drop = TRUE]])))*60)+200, 
           height = (length(GENES_LIST[[currentListName]])*60)+200)
      print(figurePlot)
      dev.off()
      cat("\n\nFirst (computing) run. Run report generation a second time to get all tables/figures and proper layout...")
    }
...
## @knitr heterogeneity_gene_expression_violin_byHTO

for(currentListName in names(GENES_LIST))
{
  cat(paste0("\n\n#### ",  currentListName, " {.tabset .tabset-fade} \n\n"))
  
  invisible( lapply( GENES_LIST[[currentListName]], violinFeatureByMetadata, seuratObject = sc10x, slot = "data", yLabel = "Norm Score", addStats = FALSE, trimTitle = 0));

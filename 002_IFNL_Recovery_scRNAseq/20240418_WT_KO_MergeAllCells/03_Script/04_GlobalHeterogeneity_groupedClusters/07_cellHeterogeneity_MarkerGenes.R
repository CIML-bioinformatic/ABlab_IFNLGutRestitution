...
topMarkers_table = by( markers, factor( markers[["cluster"]]), function(x)
{
...
topMarkers_heatmap = by( markers, factor( markers[["cluster"]]), function(x)
{
  # Filter markers based on adjusted PValue
  x = x[ x[["p_val_adj"]] < FINDMARKERS_PVAL_THR, , drop = FALSE];
  # Sort by decreasing logFC

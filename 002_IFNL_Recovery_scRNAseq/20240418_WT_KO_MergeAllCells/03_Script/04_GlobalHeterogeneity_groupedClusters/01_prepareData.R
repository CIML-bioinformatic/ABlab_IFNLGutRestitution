...
# Load the list of Seurat objects created from individual analyses
seuratList = lapply(PATH_RDS_SEURAT_OBJECT_LIST, readRDS);


# Add the sample name to the otherwise common "NEGATIVE" HTO label
resNULL = lapply( names( seuratList), function(currentSampleName)
  {
    seuratList[[currentSampleName]][[HTO_METADATA_COLUMN]] <<- gsub( "Negative", paste( currentSampleName, "Neg", sep = "_"), seuratList[[currentSampleName]][[HTO_METADATA_COLUMN, drop = TRUE]])
  })


# Merge objects in a single one
sc10x = merge(seuratList[["WT"]], 
              y = seuratList[["KO"]], 
              add.cell.ids = c("WT", "KO"), 
              merge.data = TRUE) # Also merge normalized data as it has been done similarly in both dataset (but will probably be recomputed anyway...)

# Set original identity as metadata
sc10x[["orig.ident"]] = factor(gsub("_.*", "", colnames(sc10x)))
...
if(file.exists( CELLS_COORDINATES))
{
  ...
  sc10x@reductions$external = Seurat::CreateDimReducObject( key = "EXT_", 
                                                            assay = "RNA",
                                                            embeddings = as.matrix(cellsCoordinates))
...
# Create a cell metadata combining origin (batch) and its associated clustering
sc10x[["orig.cluster"]] = paste(sc10x[["orig.ident", drop = TRUE]], Idents(sc10x), sep = "_")
